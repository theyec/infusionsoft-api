...
// of the services.
// 
// This uses the access token gotten after the Oauth process to query the API services.
.class('DataContext') .define({

    __constructor__: function(token)
    {
        //  The API Key has to be set, so you just use the access token. This is required by Infusionsoft
        this.apiKey = token;
        this.token = token;
        this.client  = xmlrpc.createSecureClient({
            protocol: 'https:',
            host: 'api.infusionsoft.com',
            path :  '/crm/xmlrpc/v1?access_token=' + this.token
        });
